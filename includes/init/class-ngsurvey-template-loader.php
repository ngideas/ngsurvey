 ...
 * @author  Gary Jones
 */
class NgSurvey_Template_Loader {
	/**
	 ...
	 * @var string
	 */
	protected $filter_prefix = 'ngsurvey';
	 ...
	 * @var string
	 */
	protected $theme_template_directory = 'ngsurvey';
	 ...
	 * @var string
	 */
	protected $plugin_directory = NGSURVEY_PATH;
	 ...
	 * 
	 * @param array $config the configuration parameters to customize attributes of the template loader
	 */
	public function __construct ( $config = array() ) {
	    
	    if( isset( $config[ 'plugin_directory' ] ) ) {
	        $this->plugin_directory = $config[ 'plugin_directory' ];
	    }
	    
	    if( isset( $config[ 'theme_template_directory' ] ) ) {
	        $this->theme_template_directory = $config[ 'theme_template_directory' ];
	    }
	    
	    if( isset( $config[ 'plugin_template_directory' ] ) ) {
	        $this->plugin_template_directory = $config[ 'plugin_template_directory' ];
	    }
	    
	    if( isset( $config[ 'filter_prefix'] ) ) {
	        $this->filter_prefix = $config[ 'filter_prefix' ];
	    }
	}
	 ...
	 *                         Default is 'data'.
	 * @return NgSurvey_Template_Loader
	 */
	...
	protected function get_template_file_names( $slug, $name ) {
		$templates = array();
		if ( isset( $name ) ) {
		    $templates[] = $slug . '-' . $name . ( substr( $name, -4 ) === '.php' ? '' : '.php' );
		}
		$templates[] = $slug . ( substr( $slug, -4 ) === '.php' ? '' : '.php' );
